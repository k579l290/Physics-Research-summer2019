#define Run04Signal_cxx
#include "Run04Signal.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <TFile.h>
...
void Run04Signal::Loop()
{
//   In a ROOT session, you can do:
//      root> .L Run01Signal.C
//      root> Run01Signal t
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   if (fChain == 0) return;
	...
	TH1D *HBmass = new TH1D("", "Mass, |Eta| < 2.5", 200,0,200);
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      // if (Cut(ientry) < 0) continue;
		...
		ptotal = p1 + p2;
		double eta_t = ptotal.Eta();
		//double ptt = ptotal.Pt();
		//photon_p->Fill(ptt);
		if (TMath::Abs(eta_t) <= 2.5) {
			double m = ptotal.M();
			HBmass->Fill(m);
		}
	}
		...
		TFile f("125_Mass_2.5Eta.root", "update");
		HBmass->Write("s4");
		
		//TFile f2("125_Pt.root" , "update");
		//photon_p->Write("s1");
		
		TFile f3("Signal_eta.root", "update");
		eta_1->Write("4_p1");
		eta_2->Write("4_p2");
