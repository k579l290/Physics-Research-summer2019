...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	  ...
	  if (Photon_size >= 2) { //look at entries with >=2 photons
		...
		//fill photon 1 eta histogram
		double eta_p1= p1.Eta();
		eta_1->Fill(eta_p1);
		
		//fill photon 2 eta histogram
		double eta_p2 = p2.Eta();
		eta_2->Fill(eta_p2);
   }
		
		//write histograms to created root files
		...
		//display each histogram to separate canvases
